...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.input_fc = nn.Linear(args.input_size_decoder + self.num_params, self.model_dim)
    ...
    def forward(self,v, z,v_enc,scene_norm, sample_num):
        ...
        for i in range(self.future_frames):
            ...
            seq_out = self.out_fc(out_tmp).view(tf_out.shape[0], -1, self.output_size)
            norm_motion = seq_out.view(-1, agent_num * sample_num, seq_out.shape[-1])
            seq_out = norm_motion + v[[-1]][...,:2]
