...
def agent_aware_attention(query: Tensor,
    ...
    tgt_len, bsz, embed_dim = query.size()  #8*33,1,model_dim
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            if in_proj_weight_self is not None:
                ...
                # This is inline in_proj function with in_proj_weight and in_proj_bias
                _w = in_proj_weight_self[embed_dim:, :]# these weights are just initialized with xavier
                _b = in_proj_bias_self[embed_dim:]
    ...
    if in_proj_weight_self is not None:
        ...
        attn_weight_self_mask = torch.eye(num_agent).to(q.device) # this is the M matrix
        ...
        attn_output_weights = attn_output_weights_inter * (1-attn_weight_self_mask) + attn_output_weights_self * attn_weight_self_mask # the main formula
